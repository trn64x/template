...
const CardTable = ({cart}:{cart?:Cart}) => {
    ...
    return ( 
        ...
        { !cart || cart.items.length  == 0 ? (
            <div>Cart is empty. <Link href='/'>Go shopping now</Link></div>
        ) : (
                        ...
                        <TableBody>
                            {cart.items.map((item)=> (
                                <TableRow key={item.slug}>
                                    <TableCell>
                                        <Link href={`/product/${item.slug}`} className='flex items-center'><Image src={item.image} alt={`${item.name}`} width={50} height={50}></Image>
